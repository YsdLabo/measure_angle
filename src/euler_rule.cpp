...
class EulerRule  :  public  rclcpp::Node
{
public:
	EulerRule( ) : Node("euler_rule_node")
	{
		// サブスクライバーを作成（/imuトピックを購読）
		sub_imu_ = this->create_subscription<sensor_msgs::msg::Imu> (
			"/imu", 
			rclcpp::SensorDataQoS(),
			std::bind(&EulerRule::callback_imu, this, std::placeholders::_1)
		);
		// パブリッシャーを作成（/euler_angle_zトピックを配信）
...
private:
	...
	void  callback_imu( const  sensor_msgs::msg::Imu::SharedPtr  msg )
	{
		...
		if(!first_time_) {
			...
			angle_z_ += h * imu_raw.angular_velocity.z;
		}
		else {
			angle_z_ = 0.0;
			first_time_ = false;
...
int  main(int  argc, char**  argv)
{
	...
	rclcpp::spin(std::make_shared<EulerRule>());
	rclcpp::shutdown();
	return  0;
}
